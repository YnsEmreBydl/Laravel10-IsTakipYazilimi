    ...
    <section class="section">
      <div class="row">
        <table class="table table-striped table-responsive">
          ...
          <tbody>
            @foreach($hizmetler as $hiz)
              @foreach($hiz->hizmetmusteri as $ad)
              @if($hiz->hizmetstatus==1)
            <tr>
              <td>{{$hiz->id}}</td>
              <td>{{$hiz->hizmetad}}</td>
              <td>{{$ad->adsoyad}}</td>
              <td>{{$hiz->hizmetfiyat}} ₺</td>
              <td>{{$hiz->hizmetbaslangic}}</td>
              <td> <button style="background:darkblue; width:150px; border: none; color:white;" class="btn btn-sm"> <?php
               $tarih1 = strtotime(date('d.m.Y'));
               $tarih2 = strtotime($hiz->hizmetbitis);
               $fark = $tarih2 - $tarih1;
               echo $sonuc = floor($fark / (60 * 60 * 24));
               ?>  Gün kaldı</button></td>
               <td>  @if($hiz->hizmetstatus ==2)
                 <button style="width:150px" class="btn btn-danger btn-sm">Süresi Bitti</button>
                 @elseif($hiz->hizmetstatus ==1)
                 <button style="width:150px" class="btn btn-success btn-sm">Devam Ediyor</button>
                 @endif</td>
              <td><a href="{{route('hizmetduzenle', $hiz->id)}}" class="btn btn-outline-warning btn-sm">Düzenle</a></td>
              <td><a href="{{route('hizmetsil', $hiz->id)}}" class="btn btn-outline-danger btn-sm">Sil</a></td>
