...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Domain Listesi</h1><br>
      <nav>
        <ol class="breadcrumb">
          <td ><a href="{{route('domain')}}"> <button style="margin-left:10px" type="submit" class="btn btn-success btn-sm">Devam Eden Domainler</button></a> </td>

          <td><a href="{{route('domainsuresibitmis')}}"> <button type="submit" style="margin-left:10px" class="btn btn-warning btn-sm">Süresi Bitmiş Domainler</button></a> </td>
          <td ><a href="{{route('domainekle')}}"> <button style="margin-left:10px" type="submit" class="btn btn-primary btn-sm">Domain Ekle</button> </a></td>
    ...
    <section class="section">
      <div class="row">
        <table class="table table-striped table-responsive">
          ...
          <tbody>
            @foreach($domainler as $dom)
              @foreach($dom->domainmusteri as $ad)
              @if($dom->domainstatus==1)
            <tr>
              <td>{{$dom->id}}</td>
              <td>{{$dom->domainad}}</td>
              <td>{{$ad->adsoyad}}</td>
              ...
              <td> <button style="background:darkblue; width:150px; border: none; color:white;" class="btn btn-sm"> <?php
               ...
               <td>  @if($dom->domainstatus ==2)
                 ...
                 @endif</td>
              <td><a href="{{route('domainyenile', $dom->id)}}" class="btn btn-outline-info btn-sm">Yenile</a></td>
